...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<AuthValue>(() => ({ user, loading, async login(email, password) { const token = await api.login(email, password); setAccessToken(token.access_token); setUser({ id: token.user_id, role: token.role, email }) }, async logout() { await api.logout().catch(() => undefined); setAccessToken(null); setUser(null) } }), [user, loading])
