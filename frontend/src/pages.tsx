...
export function Products() {
  ...
  return (
    <>
      <PageHeader eyebrow="Order operations / catalog" title="Build an order"><span className="eventual">Prices quoted synchronously · stock decided asynchronously · <b className={`cache-badge cache-${(query.data?.cacheStatus ?? 'BYPASS').toLowerCase()}`}>catalog {query.data?.cacheStatus ?? 'BYPASS'}</b></span></PageHeader>
...
export function AdminInventory() {
  ...
  const [reason, setReason] = useState('operator restock')
  const adjust = useMutation({ mutationFn: () => api.adjustStock({ product_id: productID, delta, reason }), onSuccess: () => { void products.refetch(); setDelta(1) } })
  const movements = useQuery({ queryKey: ['admin', 'movements', productID], queryFn: () => api.stockMovements(productID || undefined), refetchInterval: 10_000 })
  return <>
    <PageHeader eyebrow="Operator / inventory" title="Stock and reservations"><span className="eventual">Inventory service owns this state · <b className={`cache-badge cache-${(products.data?.cacheStatus ?? 'BYPASS').toLowerCase()}`}>catalog {products.data?.cacheStatus ?? 'BYPASS'}</b></span></PageHeader>
    <section className="metric-strip compact-metrics">{products.data?.products.map(product => <article key={product.id}><span>{product.name}</span><strong>{product.available}</strong><small>available units</small></article>)}</section>
    <section className="panel adjustment-panel"><div><p className="eyebrow">Stock control</p><h2>Record a signed adjustment</h2><p className="muted">Every change is idempotent, ledgered, and rejected if available stock would become negative.</p></div><form className="adjustment-form" onSubmit={event => { event.preventDefault(); if (productID && delta !== 0) adjust.mutate() }}><label>Product<select value={productID} onChange={event => setProductID(event.target.value)}><option value="">Choose product</option>{products.data?.products.map(product => <option value={product.id} key={product.id}>{product.name} · {product.available} available</option>)}</select></label><label>Delta<input type="number" value={delta} onChange={event => setDelta(Number(event.target.value))} /></label><label>Reason<input value={reason} onChange={event => setReason(event.target.value)} /></label><button className="button" disabled={!productID || delta === 0 || adjust.isPending}>{adjust.isPending ? 'Recording…' : 'Record adjustment'}</button></form>{adjust.isError && <ErrorMessage error={adjust.error} />}{adjust.isSuccess && <p className="success-note">Adjustment recorded at {adjust.data.movement.balance_after} available units.</p>}</section>
    ...
    <section className="panel table-panel"><div className="panel-head"><div><p className="eyebrow">Stock ledger</p><h2>{movements.data?.total ?? 0} adjustments</h2></div></div><div className="data-table"><div className="table-row table-head"><span>Product</span><span>Delta</span><span>Balance</span><span>Reason</span><span>Created</span></div>{movements.data?.movements.map(item => <div className="table-row" key={item.id}><span>{item.product_id}</span><span className={item.delta < 0 ? 'negative-number' : 'positive-number'}>{item.delta > 0 ? '+' : ''}{item.delta}</span><span>{item.balance_after}</span><span>{item.reason}</span><span>{dateTime(item.created_at)}</span></div>)}</div></section>
  </>
}
